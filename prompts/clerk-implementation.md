Your task is to implement the authentication system for the Visionary App using **Clerk**.
...
These define the project standards, architecture, and authentication rules you must follow.

I will also provide the official Clerk documentation and API documentation separately. Use them to implement authentication according to the **recommended setup for the current Expo SDK version** used in this project. Make sure all Clerk packages, APIs, and configuration are compatible with the project’s Expo version.

## Scope

Implement **authentication only**.

Build the following with Clerk:
...
Follow Clerk’s recommended security practices.

## Route Protection

Configure Expo Router so that:
...
Protected routes include all authenticated areas of the app.

Unauthenticated users must not access protected routes.

If a user is not authenticated, redirect them to Sign In.

If a valid session exists, skip authentication and go directly into the app.

## Existing UI

Do **not** redesign or change the current UI.

Only make UI changes needed to support authentication, such as:

* Showing the user’s name
* Showing the user’s profile image
...
* Put authentication logic in reusable services, not directly inside screens
...
* Deep linking, if required by the current Clerk Expo docs

Do not use deprecated APIs.

Use only the current Clerk APIs that work with this Expo SDK version.

## Google Authentication

Implement Google Sign-In using Clerk’s officially recommended Expo setup.

Make sure it works on supported platforms and follows the latest Clerk guidance.

## Sign Out

Implement a complete sign-out flow.

Signing out should:

* Clear the active session
* Remove cached authenticated user data if needed
* Redirect the user to Sign In
* Block access to protected routes until the user signs in again
...
* No existing UI was unintentionally changed
* No TypeScript or linting errors remain

Focus only on authentication. Do not start database work, Supabase integration, AI features, or anything unrelated to authentication in this task.
